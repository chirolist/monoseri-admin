      ...
      <div class="container-fluid">
        ...
        <div class="fade-in">
          ...
          <div class="row">
            <div class="col-lg-12">
              <div class="card">
                <div class="card-header">総件数：<strong>{{ $customers->total() }}</strong><button class="btn btn-warning float-md-right" type="button" onclick="download_csv()">CSVダウンロード</button></div>
                <div class="card-body">
                  <table class="table table-responsive-sm table-hover table-outline mb-3">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>{{ $customer->name }}</td>
                        <td>{{ $customer->kana }}</td>
                        <td class="text-center">{{ $customer->prefecture }}</td>
                        <td class="text-center">{{ $customer->tel }}</td>
                        <td class="text-center">{{ $customer->email }}</td>
                        <td class="text-nowrap">
                          <button class="btn btn-sm btn-square btn-block btn-dark" type="button" onclick="location.href='{{ url('/customer', [$customer->id, 'edit']) }}'">編集</button>
                        </td>
                        <td class="text-nowrap">
                          <button class="btn btn-sm btn-square btn-block btn-info" type="button" onclick="location.href='{{ url('/customer', [$customer->id]) }}'">情報</button>
                        </td>
                        <td class="text-nowrap">
                          <button class="btn btn-sm btn-square btn-block btn-danger" type="button" data-toggle="modal" data-target="#dangerModal" onclick="setDelcustomerID({{ $customer->id }})">削除</button>
                        </td>
                      </tr>
                      @endforeach
                    </tbody>
                  </table>
                  {{ $customers->appends(Request::Query())->onEachSide(2)->links() }}
                </div>
              </div>
            </div>
            <!-- /.col-->
          ...
          <div class="row align-items-center mb-3">
            <div class="col-6 col-sm-4 col-md-2 col-xl mb-3 mb-xl-0"><button class="btn btn-square btn-block btn-info" type="button" onclick="location.href='{{ url('/customer/import/create') }}'">CSV一括登録</button></div>
            <div class="col-6 col-sm-4 col-md-2 col-xl mb-3 mb-xl-0"><button class="btn btn-square btn-block btn-info" type="button" onclick="location.href='{{ url('/customer/create') }}'">新規作成</button></div>
          </div>
        </div>
      </div>
    </main>
  ...
  <div class="modal-dialog modal-danger" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button class="btn btn-danger" type="button" onclick="deletecustomer()">削除</button>
      </div>
    </div>
    <!-- /.modal-content-->
  </div>
...
<script>
  function setDelcustomerID(customer_id) {
      document.formDelete.action = '/customer/' + customer_id;
  }
  ...
  function download_csv() {
      ...
      window.location.href = '/customer_csv/download' + param;
  }
</script>
